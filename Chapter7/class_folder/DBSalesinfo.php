...
eof;
	...
	public function SalectSalesinfo($salesDate, $customerID){
		// 日付と顧客IDで売上情報を摘出（更新・削除ボタン付き）
	...
	public function UpdateDetail(){
		...
		$array = array($_POST['SalesDate'],$_POST['CustomerID'],
					   $_POST['GoodsID'],$_POST['Quantity'],$_POST['id']);
		parent::executeSQL($sql,$array);
	}
...
eof;
	...
	public function TotalAmount($SalesDate, $CustomerID){
		// 伝票の合計額
		$sql = <<<eof
		SELECT sum(salesinfo.Quantity * godds.Price)
...
eof;
	...
	public function SelectSlips($salesDate){
		// 日付で摘出
		$sql = <<<eof
		SELECT distinct salesinfo.SalesDate,
